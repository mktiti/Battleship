...
class Store<T> {
    ...
    fun place(elem: T) = synchronized(lock) {
        if (storage.isEmpty()) {
            storage.add(0, elem)
            lock.notify()
        }
    ...
    fun set(elem: T) = synchronized(lock) {
        storage.clear()
        storage.add(elem)
        lock.notify()
    }

    operator fun <R> invoke(code: T.() -> R): R = synchronized(lock) {
        while (storage.isEmpty()) {
            lock.wait()
        }
    ...
    fun visitIfPresent(): T? = synchronized(lock) {
